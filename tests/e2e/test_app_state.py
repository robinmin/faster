"""
E2E tests for the App State page functionality.
Tests JSON viewer, export functionality, and data display features.
"""
...
@pytest.mark.app_state
@pytest.mark.asyncio
async def test_app_state_page_loads_correctly(auth_page: Page) -> None:
    """Test that the App State page loads with all expected elements."""
    _ = await auth_page.goto("/dev/admin")
    await auth_page.wait_for_load_state("networkidle")

    # Navigate to app_state page
    user_menu_btn = auth_page.locator("[aria-label='User menu']")
    await user_menu_btn.click()
    app_state_menu_item = auth_page.locator("ul.dropdown-content li a").filter(has_text="App State")
    await expect(app_state_menu_item).to_be_visible(timeout=2000)
...
@pytest.mark.app_state
@pytest.mark.asyncio
async def test_app_state_header_displays_correctly(auth_page: Page) -> None:
    """Test that the App State header displays correctly."""
    _ = await auth_page.goto("/dev/admin")
    await auth_page.wait_for_load_state("networkidle")
...
@pytest.mark.app_state
@pytest.mark.asyncio
async def test_app_state_responsive_layout(auth_page: Page) -> None:
    """Test App State page responsive grid layout."""
    _ = await auth_page.goto("/dev/admin")
    await auth_page.wait_for_load_state("networkidle")
...
@pytest.mark.app_state
@pytest.mark.asyncio
async def test_app_state_accessibility_features(auth_page: Page) -> None:
    """Test App State page accessibility features."""
    _ = await auth_page.goto("/dev/admin")
    await auth_page.wait_for_load_state("networkidle")
