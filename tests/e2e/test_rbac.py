"""
E2E tests for the RBAC page functionality.
Tests route access control table, statistics, and filtering.
"""
...
@pytest.mark.rbac
@pytest.mark.asyncio
async def test_rbac_page_loads_correctly(auth_page: Page) -> None:
    """Test that the RBAC page loads with all expected elements."""
    ...
    rbac_menu_item = auth_page.locator("ul.dropdown-content li a").filter(has_text="RBAC")
    await expect(rbac_menu_item).to_be_visible(timeout=2000)
    await rbac_menu_item.click()
    await auth_page.locator("[x-show*='rbac']").wait_for(state="visible", timeout=5000)

    # Verify rbac page is active
    rbac_view = auth_page.locator("[x-show*='rbac']")
    await expect(rbac_view).to_be_visible()


@pytest.mark.rbac
@pytest.mark.asyncio
async def test_rbac_header_displays_correctly(auth_page: Page) -> None:
    """Test that the RBAC header displays correctly."""
    _ = await auth_page.goto("/dev/admin")
    await auth_page.wait_for_load_state("networkidle")
...
@pytest.mark.rbac
@pytest.mark.asyncio
async def test_rbac_responsive_layout(auth_page: Page) -> None:
    """Test RBAC page responsive grid layout."""
    _ = await auth_page.goto("/dev/admin")
    await auth_page.wait_for_load_state("networkidle")
...
@pytest.mark.rbac
@pytest.mark.asyncio
async def test_rbac_accessibility_features(auth_page: Page) -> None:
    """Test RBAC page accessibility features."""
    _ = await auth_page.goto("/dev/admin")
    await auth_page.wait_for_load_state("networkidle")
